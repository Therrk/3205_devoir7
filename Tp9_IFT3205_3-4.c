...
void apply_the_correction_filter(float * signal_r, int length) {
    ...
    for (int i = 0;i < length;i++){
        ...
        complex y = complex_inv(complex_mul(complex_add(r2c(1.0),
                                                        complex_mul(r2c(-2.0),
                                                                    complex_inv(z))),
                                            complex_add(r2c(1.0),
...
int main(int argc,char **argv)
{
    ...
    float * signal_r = LoadSignalDat("FilteredSoundFile", &length);

    apply_the_correction_filter(signal_r, length);
    SaveSignalDat("UnfilteredSoundFile", signal_r, length);
    
    // On voit dans le fichier `impulse_response.dat` que le signal ne progresse
    // pas vers zéro. Conséquemment, il est instable.
